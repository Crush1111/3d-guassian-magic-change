        ...
        if low_memory:
            ...
            if using_depth:
                if extr.name.endswith('JPG'):
                    depth_path = os.path.join(images_folder.replace('images', 'depth'), os.path.basename(extr.name).replace('JPG', 'jpg'))
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                            image_path=image_path, image_name=image_name, width=contents["w"], height=contents["h"], depth=depth))
...
def readCamerasFromTransformsFile(path, transformsfile, extension="", using_depth=False, low_memory=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            ## 坐标系变换，但是没有看懂这个是在设么坐标系下
            # matrix = np.array(frame["transform_matrix"])
            # matrix = matrix[np.array([1, 0, 2, 3]), :]
            # matrix[:, 1:3] *= -1
            # matrix[2, :] *= -1
            #
            # R = matrix[:3, :3]
