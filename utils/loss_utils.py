...
def compute_rank_loss(dyn_depth, gt_depth, lambda_depth, sample_nums=100):
    ...
    # 直接满足前后关系
    mask_rank = torch.where(gt_depth.unsqueeze(-1) - gt_depth.unsqueeze(1) > 1e-1, 1, 0).type(torch.bool)
...
def compute_continue_loss(dyn_depth, gt_depth, lambda_depth, sample_nums=100, patch_size=3):
    ...
    mask = torch.logical_and(condition <= 1e-2, condition > 0)
    if torch.all(~mask):
        continue_loss = torch.tensor(0).to(dyn_depth.devices)
        return continue_loss
    else:
        continue_loss = ((pred_depth[:, 0, :].unsqueeze(0).permute(1, 0, 2) - pred_depth).abs() - 1e-2)[mask].clamp(0).mean() * lambda_depth
        return continue_loss
